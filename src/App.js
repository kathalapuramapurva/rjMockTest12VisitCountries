...
import CountryNameItem from './components/CountryNameItem'
import './App.css'
...
class App extends Component {
  ...
  markVisited = givenId => {
    ...
    const updatedList = visitedCountriesList.map(eachCountry => {
      if (eachCountry.id === givenId) {
        return {...eachCountry, isVisited: true}
      }
      return eachCountry
    })
    this.setState({visitedCountriesList: updatedList})
  }

  removeVisitedCountry = givenId => {
    const {visitedCountriesList} = this.state
    const updatedList = visitedCountriesList.map(eachCountry => {
      if (eachCountry.id === givenId) {
  ...
  render() {
    const {visitedCountriesList} = this.state
    const visitedList = visitedCountriesList.filter(
      eachCountry => eachCountry.isVisited === true,
    )
    return (
      <div className="app-container">
        <div className="content-container">
          <h1 className="app-heading">Countries</h1>
          <ul className="country-names-list">
          ...
          <h1 className="app-heading">Visited Countries</h1>
          <div className="bottom-container">
            {visitedList.length === 0 ? (
              <p className="no-countries-visited">No Countries Visited Yet</p>
            ) : (
              <ul className="countries-visited-list">
                {visitedList.map(eachCountry => (
                  <VisitedItem
                    key={eachCountry.id}
                    visitedDetails={eachCountry}
